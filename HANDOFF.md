...
### 後台「＋人工單」UI/UX 調整（快速開櫃）
- 各欄位改**非必填**（除「自選格子」外），表單上方加提醒。
- 新增「寄物／快速開櫃」：`stringId=0` → 走隱藏線種「寄物」（is_active=FALSE，不進客人選單與線種管理），不驗磅數／顏色，**只選格子即可直接開櫃＋印貼紙**。
- 會員欄改「索引搜尋」：輸入名字／電話即時模糊搜尋歷史客人（`GET /api/customers/search?q=`），點選自動帶入名字＋電話。
- 修：電話欄太窄顯示少一位（9 碼）→ 加寬；`customerName` 合併改 `filter(Boolean).join(' · ')`，避免空名字時出現多餘「·」。
- 已知小尾巴：寄物標籤第 2 行會印「0 lbs」（kiosk poller `$L.tension lbs`），暫不影響；要改需連 kiosk 腳本一起更新。
